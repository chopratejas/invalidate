"""POST /api/check {"facts": "...", "events": "..."} -> {"ok": true, "facts": [...], "steps": [...], "summary": {...}}

Stateless: a throwaway in-memory Invalidate per request, judged by Jev. Nothing is persisted.
Hard caps live in invalidate.ui.check (MAX_FACTS / MAX_EVENTS / MAX_LINE); this file adds a body
cap and a small per-instance, per-IP rate limit so the public playground can't be hammered.
"""
import json
import os
import sys
...
from invalidate.ui.check import check  # noqa: E402
...
def _typesafe_error_types():
    ...
    except Exception:  # SDK missing or renamed; fall through to the generic 500
        return ()
...
class handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):  # noqa: N802
        self._err(405, "POST a JSON body: {\"facts\": \"...\", \"events\": \"...\"}")
    ...
    def do_POST(self):  # noqa: N802
        ...
        if length <= 0:
            return self._err(400, "Empty body. Send JSON: {\"facts\": \"...\", \"events\": \"...\"}")
        if length > MAX_BODY:
            return self._err(413, f"Body too large (max {MAX_BODY // 1024} KB).")

        if _rate_limited(self._client_ip()):
            return self._err(429, f"Easy there. The playground allows {RATE_LIMIT} checks per "
        ...
        if not isinstance(data, dict):
            return self._err(400, "Body must be a JSON object with \"facts\" and \"events\".")
        facts, events = data.get("facts", ""), data.get("events", "")
        if not isinstance(facts, str) or not isinstance(events, str):
            return self._err(400, "\"facts\" and \"events\" must be strings (one item per line).")
        ...
        try:
            result = check(facts, events, judge)
